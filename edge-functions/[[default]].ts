...
// ========== 配置 ==========
const CACHE_VERSION = "v8"; // ★ 版本号，修改配置后可手动增加强刷
...
const RATE_LIMIT = 30;// ★ 分组显示顺序
const ORDERED_GROUPS = [
"高清", "央视", "卫视", "少儿", "音乐", "动漫",
"戏曲", "纪录片", "体育", "电影", "新闻",
"互联网动漫", "互联网电影", "互联网电视剧"
];// ========== 工具函数 ==========function normalizeCCTV(name: string): string | null {
...
const tagPattern = /$$([^$$]+)]|$(4K\vert{}1080P\vert{}720P\vert{}HEVC\vert{}H265\vert{}H264\vert{}高清\vert{}超清\vert{}蓝光\vert{}HD\vert{}SD\vert{}UHD\vert{}FHD\vert{}HDR\vert{}杜比\vert{}DOLBY)$/gi;
const tags: string[] = [];
let match;
...
async function fetchAndBuild(sources: string[], chanAlias: Map, kv: any): Promise<{ m3u: string; apiData: any }> {
let blacklist: Set = new Set();
try {
const bl = await kv.get(BLACKLIST_KEY, "json");
if (bl && Array.isArray(bl)) blacklist = new Set(bl);
} catch {}const groups: Record>> = {};
const chanMeta: Record = {};function addToGroup(group: string, stdName: string, url: string) {
...
let curTvgId = "";
let curTvgLogo = "";
let curTvgShift = "";for (const line of lines) {
...
else chanNames.sort((a, b) => a.localeCompare(b));for (const chan of chanNames) {
  for (const url of chanMap[chan]) {
...
function renderEcoHomePage(): string {
return `湿地生态保护中心 | Wetland Conservation Center`;
}// ========== 入口处理 ==========
export async function onRequest(context: any) {
const { request, env } = context;
const url = new URL(request.url);
const path = url.pathname;// 避免未定义报错
const kv = env?.IPTV_KV;
if (!kv) return new Response("ERROR: IPTV_KV 未配置绑定", { status: 500 });// Rate Limit
...
if (key !== env.AUTH_KEY) return new Response("Unauthorized", { status: 403 });  const sources = (env.SOURCES || "").split(",").map((s: string) => s.trim()).filter(Boolean);
...
}// 3. M3U 订阅接口 (修改响应头为 text/plain，防止触发浏览器播放插件)
if (path === "/iptv.m3u") {
const key = url.searchParams.get("key");
if (key !== env.AUTH_KEY) return new Response("Unauthorized", { status: 403 });const sources = (env.SOURCES || "").split(",").map((s: string) => s.trim()).filter(Boolean);
...
if (isFresh) {
  return new Response(cached.m3u, { headers: m3uHeaders });
}
...
if (!epgUrl) return new Response("", { headers: { "Content-Type": "application/xml" } });
try {
const controller = new AbortController();
...
}// 6. Player API (支持 Xtream API 播放器格式)
...
return new Response(renderEcoHomePage(), {
