...
// Declare global KV namespace for TypeScript environment
declare const IPTV_KV: any;
...
// ========== 拉取源 + 聚合 ==========
// ★ 保留：epgUrl 参数，生成标准 M3U 头（含 x-tvg-url）
async function fetchAndBuild(sources: string[], chanAlias: Map<string, string>, kv: any, epgUrl?: string): Promise<{ m3u: string; apiData: any }> {
  ...
  try {
    ...
    const bl = typeof blRaw === "string" ? JSON.parse(blRaw) : blRaw;
...
function getKV(env: any) {
  if (typeof IPTV_KV !== "undefined") return IPTV_KV;
  if (env && env.IPTV_KV) return env.IPTV_KV;
  return null;
}
...
export async function onRequest(context: any) {
  ...
  // 3. M3U 订阅导出（★ 方案A：UA 判断，浏览器看文本，播放器拿标准 M3U）
  if (path === "/iptv.m3u") {
    const key = url.searchParams.get("key");
    if (key !== env.AUTH_KEY) return new Response("Unauthorized", { status: 403, headers: CORS_HEADERS });
    const sources = (env.SOURCES || "").split(",").map((s: string) => s.trim()).filter(Boolean);
    ...
    // ★ UA 判断：浏览器显示纯文本，IPTV 播放器使用标准 M3U Header
